...
#  @sut = MATTI.sut(:Id =>'sut_qt') # for Qt, id in configuration file sut_qt
#
...
class MATTI
	...
	# MobyUtil::ParameterFileNotFoundError:: if paramter file 'matti_home'/tdriver_parameters.xml is not found
	# === returns
	# Object:: Object that SutFactory returns 
	# === example
	#  @sut = MATTI.connect_sut(:Id =>'sut_qt') # for qt, id in configuration file sut_qt
	...
	# Object:: SUT object 
	# === example
	#  @sut = MATTI.disconnect_sut(:Id =>'sut_qt') # for qt, should be connected already
	...
	# ArgumentError:: Sut id not given
	# === example
	#  @sut = MATTI.reboot_sut(:Id =>'sut_qt') # for Qt, should be connected already   
	...
	# Wrapper for SUT functionality. For documentation, please see MATTI::connect_sut
	def self.sut( *args )
